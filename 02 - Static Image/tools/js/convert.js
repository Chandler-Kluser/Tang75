...
function convertTo3Bit(r, g, b) {
  return (quantizeColor(r) << 2) | (quantizeColor(g) << 1) | quantizeColor(b);
}
...
function generateVerilogROM(array, moduleName) {
    let verilogCode = `module ${moduleName}(input [9:0] addr, output reg [2:0] data = 0);\n\n`;
    ...
    array.forEach((value, index) => {
        ...
        const bitsReversed = binaryValue.split('').reverse().join('');
        verilogCode += `            10'b${binaryAddress}: data <= 3'b${bitsReversed};\n`;
    });
